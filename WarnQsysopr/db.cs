...
namespace dblib
{
    ...
    public class DBConnection
    {
        ...
        private String IBMI = "iseries";
        private String msSqlServer = "myserver";
        private String msSqlServerUsername = "sa";
        private String msSqlServerPassword = "sapassword";

        public DBConnection(String dbtype = "IBMI")
        {
            ...
            switch (dbtype)
            {
                case "IBMI":
                    ...
                    (conn as ADODB.Connection).Open($"Driver=IBM i Access ODBC Driver;QueryTimeOut=0;System={IBMI};Uid={UserID};Pwd={Password};Translate Binary=true;",
                            UserID, Password, 0);
                    rdr = new ADODB.Recordset();
                    (rdr as ADODB.Recordset).CursorLocation = ADODB.CursorLocationEnum.adUseClient;
                    break;
                case "MSSQL":
                    conn = new SqlConnection($"server={msSqlServer};user id={msSqlServerUsername};password={msSqlServerPassword};initial catalog=TMWAMS");
                    break;
            }
            inUse = true;
        }
    ...
    public class DBManager
    {
        ...
        public DBConnection getConnObject(String ctype = "IBMI")
        {
            ctype = ctype.ToUpper();
